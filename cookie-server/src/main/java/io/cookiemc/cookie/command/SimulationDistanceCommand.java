...
import io.cookiemc.cookie.server.level.distance.WorldSpecificViewDistancePersistentState;
import io.cookiemc.cookie.server.level.distance.command.CommandUtils;
    ...
    public LiteralCommandNode<CommandSourceStack> register(@NotNull CommandDispatcher<CommandSourceStack> commandDispatcher) {
        return commandDispatcher.register(
            literal("simulationdistance")
                .requires((src) -> src.hasPermission(3, "cookie.world.command.simulationdistance"))
                .then(literal("set")
                    .then(literal("global")
