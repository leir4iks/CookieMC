package io.cookiemc.cookie;

import io.cookiemc.cookie.config.AnnotationBasedYamlSerializer;
import io.cookiemc.cookie.config.ConfigHandlers;
import io.cookiemc.cookie.config.ConfigSerializer;
import io.cookiemc.cookie.config.Configuration;
import io.cookiemc.cookie.config.ConfigurationUtils;
import io.cookiemc.cookie.config.RuntimeModifier;
import io.cookiemc.cookie.config.SerializationBuilder;
import io.cookiemc.cookie.config.annotation.AlwaysAtTop;
import io.cookiemc.cookie.config.annotation.Comment;
import io.cookiemc.cookie.config.annotation.Experimental;
import io.cookiemc.cookie.config.annotation.numeric.NonNegativeNumericValue;
import io.cookiemc.cookie.config.annotation.numeric.PositiveNumericValue;
import io.cookiemc.cookie.config.annotation.numeric.Range;
import io.cookiemc.cookie.config.impl.ConfigAccess;
import io.cookiemc.cookie.config.internal.ConfigurationManager;
import io.cookiemc.cookie.entity.MultithreadedTracker;
import io.cookiemc.cookie.entity.pathfinding.PathfindTaskRejectPolicy;
import io.cookiemc.cookie.util.YamlTextFormatter;
...
public class Config {
    ...
    @Comment(value = {
        ...
        "With cookie' multi-threaded context, plugins most likely are not going to be as compatible with",
        "cookie, given it can fire events basically anywhere at anytime. Because of this, some plugins break.",
            ...
            public static void post() {
                ...
                for (String name : INSTANCE.entities.dynamicActivationofBrain.blackedEntities) {
                    ...
                    EntityType.byString(typeId).ifPresentOrElse(entityType ->
                        ...
                        () -> cookieBootstrap.LOGGER.warn("Skip unknown entity {}, in {}", typeId, "entities.dynamicActivationofBrain.blackedEntities.blacklisted-entities")
                    );
                }
            }
        }
    ...
    private static <T extends Config> @NotNull ConfigSerializer<T> buildSerializer(Configuration config, Class<T> configClass) {
        ...
        return new AnnotationBasedYamlSerializer<>(SerializationBuilder.<T>newBuilder()
            .header(new String[]{
                "This is the main cookie configuration file",
                ...
                "join our discord at https://cookiemc.io/discord",
            ...
            .runtimeModifier("debug.*", new RuntimeModifier<>(boolean.class, (original) -> cookieBootstrap.RUNNING_IN_IDE || original))
            .post(context -> {
                ...
                if (INSTANCE.debug.printConfigurationTree) {
                    ...
                    cookieBootstrap.LOGGER.info(Component.text("Printing configuration tree:").appendNewline().append(formatter.apply(context.contents())));
                }
                for (final String change : changes) {
                    cookieBootstrap.LOGGER.info(change);
                }
                ...
                if (INSTANCE.entities.entityTracking.enabled) {
                    ...
                    cookieBootstrap.LOGGER.info("Using {} threads for Async Entity Tracker", INSTANCE.entities.entityTracking.asyncEntityTrackerMaxThreads);
                } else {
                ...
                if (INSTANCE.chunks.nativeAcceleration.nativeAccelerationEnabled) {
                    try {
                        ...
                        Class.forName("io.cookiemc.cookie.util.NativeLoader").getField("lookup").get(null);
                    } catch (Throwable t) {
                        cookieBootstrap.LOGGER.error("Couldn't load NativeLoader", t);
                    }
                }
    ...
    public static Config init() {
        ...
        cookieBootstrap.LOGGER.info("Finished cookie config init in {}ms", TimeUnit.MILLISECONDS.convert(Util.getNanos() - startNanos, TimeUnit.NANOSECONDS));
        return INSTANCE;
    }
