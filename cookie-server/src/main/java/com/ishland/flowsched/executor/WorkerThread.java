...
public class WorkerThread extends TickThread { // cookie - extend TickThread

    private static final Logger LOGGER = LoggerFactory.getLogger("FlowSched Executor Worker Thread");

    private final ExecutorManager executorManager;
    private final AtomicBoolean shutdown = new AtomicBoolean(false);
    public volatile boolean active = false; // cookie - add activity boolean for api

    public WorkerThread(ExecutorManager executorManager) {
        super("null_worker"); // cookie - extend TickThread
    ...
    public void run() {
        ...
        while (true) {
            ...
            active = true; // cookie - add activity boolean for api
            if (pollTasks()) {
                continue;
            }
...
//            LockSupport.parkNanos("Waiting for tasks", 1_000_000); // 1ms
            synchronized (this.executorManager.workerMonitor) {
                ...
                try {
                    active = false; // cookie - add activity boolean for api
