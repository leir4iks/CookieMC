...
public abstract class BaseRepresenter {
    protected final Map<Class<?>, io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.representer.Represent> representers = new HashMap();
    protected final Map<Class<?>, io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.representer.Represent> multiRepresenters = new LinkedHashMap();
    protected final Map<Object, Node> representedObjects;
    protected io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.representer.Represent nullRepresenter;
    ...
    protected final Node representData(Object data) {
        ...
        if (this.representedObjects.containsKey(this.objectToRepresent)) {
            ...
            if (this.representers.containsKey(clazz)) {
                io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.representer.Represent representer = this.representers.get(clazz);
                node = representer.representData(data);
            } else {
                for (Class<?> repr : this.multiRepresenters.keySet()) {
                    if (repr != null && repr.isInstance(data)) {
                        io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.representer.Represent representer = this.multiRepresenters.get(repr);
                        node = representer.representData(data);
                        return node;
                    }
                }
                ...
                if (this.multiRepresenters.containsKey(null)) {
                    io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.representer.Represent representer = this.multiRepresenters.get(null);
                    node = representer.representData(data);
                } else {
                    io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.representer.Represent representer = this.representers.get(null);
                    node = representer.representData(data);
                }
            }
