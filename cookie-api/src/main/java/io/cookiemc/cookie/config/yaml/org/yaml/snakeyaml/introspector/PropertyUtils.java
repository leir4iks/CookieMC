...
package io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.introspector;

import io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.error.YAMLException;
import io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.util.PlatformFeatureDetector;
                    ...
                    for (Class<?> c = type; c != null; c = c.getSuperclass()) {
                        for (Field field : c.getDeclaredFields()) {
                            ...
                            if (!Modifier.isStatic(modifiers) && !Modifier.isTransient(modifiers) && !properties.containsKey(field.getName())) {
                                properties.put(field.getName(), new io.cookiemc.cookie.config.yaml.org.yaml.snakeyaml.introspector.FieldProperty(field));
                            }
                        }
                    }
                    break;
